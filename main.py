...
def dump(split, classes_txt_path, sub_classes_txt_path, image_height, image_width, fps, output_dir_path):
    ...
    with open(sub_classes_txt_path, 'r') as f:
        sub_classes = f.readlines()
    sub_classes = [label.strip() for label in sub_classes]
    ...
    for data_index, data in tqdm.tqdm(enumerate(ds), total=len(ds), desc='dump videos'):
        ...
        if class_label not in sub_classes:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sub_classes_txt_path', type=str,
                        default=os.path.join(os.path.dirname(__file__), 'sub_ucf101_labels.txt'))
    ...
    dump(args.split, args.classes_txt_path, args.sub_classes_txt_path, args.image_height, args.image_width, args.fps, args.output_dir_path)
